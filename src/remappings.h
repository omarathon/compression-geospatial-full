...
std::vector<int32_t> RemapToMortonOrder(const std::vector<int32_t>& input,
                                        int N) {
  if (input.size() != N * N) {
  ...
  for (int y = 0; y < N; ++y) {
    for (int x = 0; x < N; ++x) {
      ...
      if (mortonCode > maxIndex) {
        std::cerr << "Morton remap error: Morton code out of bounds - "
                  << mortonCode << std::endl;
        throw std::out_of_range(
            "Morton remap code exceeds the bounds of the output array.");
      }
...
std::vector<int32_t> RemapToZigzagOrder(const std::vector<int32_t>& input,
                                        int N) {
