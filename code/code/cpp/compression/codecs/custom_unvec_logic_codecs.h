...
public:
    ...
    void encodeArray(const int32_t *in, const size_t length) override {
        if (length > 0) {
            // Store the first element as-is (consider ZigZag encoding here as well if needed)
            compressed_data.push_back(in[0]);
            for (size_t i = 1; i < length; ++i) {
                // Compute delta
                int32_t delta = in[i] - in[i - 1];
                // Apply ZigZag encoding directly to delta
                uint32_t zigzagged = (delta << 1) ^ (delta >> 31);
                // Store the ZigZag encoded delta
    ...
    void decodeArray(int32_t *out, const size_t length) override {
        if (length > 0) {
            // Decode the first element as-is (consider ZigZag decoding here as well if applied to the first element)
            out[0] = compressed_data[0];
            for (size_t i = 1; i < length; ++i) {
                // Apply ZigZag decoding directly to the stored value
                uint32_t zigzagged = static_cast<uint32_t>(compressed_data[i]); // Cast to uint32_t if your vector is of type int32_t
...
template<typename T>
class DictCodec : public StatefulIntegerCodec<int32_t> { // Note the change here
    static_assert(std::is_integral<T>::value, "Dictionary index type must be an integral type.");
...
public:
    ...
    void encodeArray(const int32_t *in, const size_t length) override {
        for (size_t i = 0; i < length; ++i) {
            ...
            T index = dict.at(in[i]);
            this->compressed_data.push_back(index); // Store index as type T
        }
    }
    ...
    void decodeArray(int32_t *out, const size_t length) override {
        for (size_t i = 0; i < length; ++i) {
            ...
            out[i] = reverseDict[index]; // Lookup the original value
        }
    }
...
public:
    ...
    void encodeArray(const int32_t *in, const size_t length) override {
        ...
        // Compute reference value (e.g., minimum of the array)
