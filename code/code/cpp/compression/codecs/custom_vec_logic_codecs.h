...
//         if (length < 4) {
//             compressed_data.insert(compressed_data.end(), in, in + length);
//             return;
//         }

//         // Copy the first four elements directly
//         compressed_data.insert(compressed_data.end(), in, in + 4);

//         size_t i = 4;

//         for (; i < length - 4; i += 4) {
//             __m128i current = _mm_loadu_si128(reinterpret_cast<const __m128i*>(in + i));
//             __m128i prev = _mm_loadu_si128(reinterpret_cast<const __m128i*>(in + i - 4));

//             __m128i delta = _mm_sub_epi32(current, prev);
//             int32_t buffer[4];
//             _mm_storeu_si128(reinterpret_cast<__m128i*>(buffer), delta);

//             compressed_data.insert(compressed_data.end(), buffer, buffer + 4);
//         }
...
//         for (; i < length; ++i) {
//             compressed_data.push_back(in[i] - in[i - 4]);
//         }
//     }

//     void decodeArray(int32_t* out, const size_t length) override {
//         if (length < 4) {
//             std::copy(compressed_data.begin(), compressed_data.end(), out);
//             return;
//         }

//         // Copy the first four elements directly
//         std::copy(compressed_data.begin(), compressed_data.begin() + 4, out);

//         size_t i = 4;

//         for (; i < length - 4; i += 4) {
//             __m128i delta = _mm_loadu_si128(reinterpret_cast<const __m128i*>(compressed_data.data() + i));
//             __m128i prev = _mm_loadu_si128(reinterpret_cast<const __m128i*>(out + i - 4));
...
//         // Handle any remaining elements at the end
//         for (; i < length; ++i) {
//             out[i] = compressed_data[i] + out[i - 4];
//         }
//     }
...
public:
    void encodeArray(const int32_t* in, const size_t length) override {
        ...
        // Perform zig-zag encoding directly into compressed_data
        compressed_data[0] = (in[0] << 1) ^ (in[0] >> 31); // Store the first element as is, after zig-zag encoding

        __m128i prev = _mm_set1_epi32(in[0]); // Initialize prev with the first element
        size_t i = 1;
        for (; i < length - 4; i += 4) {
            __m128i current = _mm_loadu_si128(reinterpret_cast<const __m128i*>(in + i));
            __m128i delta = _mm_sub_epi32(current, prev);

            // Zig-zag encode the delta values
            __m128i shift1 = _mm_slli_epi32(delta, 1); // delta << 1
            __m128i shiftR = _mm_srai_epi32(delta, 31); // arithmetic shift right to propagate the sign bit
            ...
            prev = _mm_shuffle_epi32(current, _MM_SHUFFLE(3, 3, 3, 3)); // Set prev to the last element of current for the next iteration
        }

        // Handle remaining elements
        for (; i < length; ++i) {
            int32_t delta = in[i] - in[i - 1];
            compressed_data[i] = (delta << 1) ^ (delta >> 31); // Zig-zag encode
        }
    }
    ...
    void decodeArray(int32_t* out, const size_t length) override {
        ...
        // Decode the first element from zig-zag encoding
        ...
        for (; i < length - 4; i += 4) {
            ...
            // Zig-zag decode
            __m128i shiftR = _mm_srai_epi32(zigzag, 1); // Logical shift right
        ...
        // Handle remaining elements
        for (; i < length; ++i) {
            int32_t delta = (compressed_data[i] >> 1) ^ (-(compressed_data[i] & 1)); // Zig-zag decode
            out[i] = delta + out[i - 1];
        }
    }
...
//         // Copy the first eight elements directly
//         compressed_data.insert(compressed_data.end(), in, in + 8);

//         size_t i = 8;
//         for (; i < length - 8; i += 8) {
//             __m256i current = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(in + i));
//             __m256i prev = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(in + i - 8));

//             __m256i delta = _mm256_sub_epi32(current, prev);
//             int32_t buffer[8];
//             _mm256_storeu_si256(reinterpret_cast<__m256i*>(buffer), delta);

//             compressed_data.insert(compressed_data.end(), buffer, buffer + 8);
//         }

//         // Handle any remaining elements at the end
//         for (; i < length; ++i) {
//             compressed_data.push_back(in[i] - in[i - 8]);
//         }
//     }
...
//         // Copy the first eight elements directly
//         std::copy(compressed_data.begin(), compressed_data.begin() + 8, out);

//         size_t i = 8;
//         for (; i < length - 8; i += 8) {
//             __m256i delta = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(compressed_data.data() + i));
//             __m256i prev = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(out + i - 8));
...
//         // Handle any remaining elements at the end
//         for (; i < length; ++i) {
//             out[i] = compressed_data[i] + out[i - 8];
//         }
//     }
...
public:
    void encodeArray(const int32_t* in, const size_t length) override {
        ...
        // Copy the first eight elements directly and apply zig-zag encoding.
        ...
        for (; i < length - 8; i += 8) {
            ...
            // Apply zig-zag encoding to delta values.
            __m256i sign = _mm256_srai_epi32(delta, 31); // Arithmetic shift right to propagate the sign bit.
            ...
            prev = _mm256_permute4x64_epi64(current, _MM_SHUFFLE(3, 3, 3, 3)); // Prepare 'prev' for next iteration.
        }

        // Handle any remaining elements at the end, applying zig-zag encoding.
        for (; i < length; ++i) {
            int32_t delta = in[i] - in[i - 1];
    ...
    void decodeArray(int32_t* out, const size_t length) override {
        ...
        // Decode the first eight elements directly from zig-zag encoding.
        for (size_t i = 0; i < 8; ++i) {
...
public:
    ...
    void encodeArray(const int32_t* in, const size_t length) override {
        ...
        size_t i = 0;
        if (length >= 16) {
            // Copy the first sixteen elements directly and apply zig-zag encoding.
            ...
            for (; i < length - 16; i += 16) {
                ...
                prev = _mm512_permutexvar_epi32(_mm512_set1_epi32(15), current); // Prepare 'prev' for next iteration.
            }
        }
        // Handle any remaining elements at the end, applying zig-zag encoding.
        for (; i < length; ++i) {
    ...
    void decodeArray(int32_t* out, const size_t length) override {
        ...
        if (length >= 16) {
            // Decode the first sixteen elements directly from zig-zag encoding.
            for (size_t i = 0; i < std::min(size_t(16), length); ++i) {
                int32_t value = compressed_data[i];
                out[i] = (value >> 1) ^ (-(value & 1));
            }
...
private:
    ...
    std::unordered_map<int32_t, T>& dict; // Mapping values to indexes of type T
    std::vector<int32_t>& reverseDict; // Mapping indexes back to values
...
public:
    ...
    void encodeArray(const int32_t *in, const size_t length) override {
        ...
        for (size_t i = 0; i < length; ++i) {
            ...
            // std::cout << "ex " << (int)index << std::endl;
...
public:
    ...
    void encodeArray(const int32_t* in, const size_t length) override {
        ...
        // Compute the minimum (reference) value in a vectorized manner
        ...
        compressed_data[0] = referenceValue; // Store the reference value at the start
        ...
        for (; i < length - 4; i += 4) {
            ...
            _mm_storeu_si128(reinterpret_cast<__m128i*>(&compressed_data[i + 1]), zigzagEncoded); // +1 to skip the reference value
        }
        for (; i < length; ++i) {
            int32_t diff = in[i] - referenceValue;
            // Inline Zig-Zag encoding for remaining elements
    ...
    void decodeArray(int32_t* out, const size_t length) override {
        ...
        int32_t referenceValue = compressed_data[0]; // Retrieve the reference value
        __m128i refValVec = _mm_set1_epi32(referenceValue);

        int i = 0;
        for (; i < length - 4; i += 4) {
            __m128i zigzagEncoded = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&compressed_data[i + 1])); // +1 to skip the reference value
        ...
        for (; i < length; ++i) { // Adjust for the offset by 1 for the reference value
...
public:
    ...
    void encodeArray(const int32_t* in, const size_t length) override {
        ...
        // Vectorized computation of minimum
        ...
        // Compute minimum from vector
        int32_t minBuffer[8];
        _mm256_storeu_si256(reinterpret_cast<__m256i*>(minBuffer), minValVec);
        int32_t referenceValue = std::min({minBuffer[0], minBuffer[1], minBuffer[2], minBuffer[3],
                                           minBuffer[4], minBuffer[5], minBuffer[6], minBuffer[7]});
        // Handle remaining elements for min computation
        for (; i < length; ++i) {
            referenceValue = std::min(referenceValue, in[i]);
        }

        // Prepare compressed_data with the size to include referenceValue
...
public:
    ...
    void encodeArray(const int32_t* in, const size_t length) override {
        ...
        // Vectorized computation of minimum using AVX-512
        ...
        // Store reference value at the beginning of compressed data
        compressed_data[0] = referenceValue;
...
public:
    ...
    void encodeArray(const int32_t *in, const size_t length) override {
        ...
        while (i < length) {
            ...
            // Vectorized comparison for adjacent elements
    ...
    void decodeArray(int32_t *out, const size_t length) override {
        ...
        for (size_t i = 0; i < compressed_data.size(); i += 2) {
            ...
            // Vectorized filling of output array
...
public:
    ...
    void encodeArray(const int32_t *in, const size_t length) override {
        ...
        while (i < length) {
            ...
            // Vectorized comparison for adjacent elements using AVX2
