...
bool test_codec(std::vector<int32_t>& data, StatefulIntegerCodec<int32_t>& codec) {
    ...
    // NOTE: 2ibench codecs require the +8, otherwise corrupted size vs. prev_size at end of program :thinking:
    ...
    // print data
    // std::cout << "data_back" << std::endl;
    // for (int i = 0; i < data.size(); i++) {
    //     std::cout << data_back[i] << ",";
    // }
    // std::cout << std::endl;
    // std::cout << "data_back_unsigned" << std::endl;
    // for (int i = 0; i < data.size(); i++) {
    //     std::cout << data_back_unsigned[i] << ",";
    // }
    // std::cout << std::endl;
    ...
    uint64_t sum_back = (static_cast<uint64_t>(data_back_unsigned[data.size() + 1]) << 32) | data_back_unsigned[data.size()];
    std::cout << "expected: " << sum << ", actual: " << sum_back << std::endl;
    if (sum != sum_back) {
        std::cerr << "sum not equal " << codec.name() << std::endl;
        return false;
    }
...
int main(int argc, char* argv[]) {
    ...
    if (range == 0) { // Maximum range.
        ...
        max = pow(2,28) - 1;
        // uint32_t val = pow(2,28) - 1 - i;
        // data_large.push_back((int32_t)(val));
    }
    else if (range == 2) {
    ...
    for (int i = 0; i < n_val; i++) {
        ...
        // max:
        // data_large.push_back(0x7FFFFFFF - i);
        // data_large.push_back(0x80000000 + i);

        

        // even smaller for 2ibench_interpolativeblock
        // from testing, it works up to 2^26. but, 2ibench's test only tests up to 2^24: https://github.com/jermp/2i_bench/blob/df6be0c98b63ff85938cc4992e1e031afa8df244/test/test_block_codecs.cpp#L13
        // NOTE: we might want to just implement the codecs for uint32s...
        // uint32_t val = pow(2,24) - 1 - i;
        // data_large.push_back((int32_t)(val));
        // val = i + 1;
        // data_large.push_back((int32_t)(val));

        // data_large.push_back(-1);

        
        // data_large.push_back(999999 - i);
        // data_large.push_back(999999 + i);
    }
    ...
    auto createAndAddCodecs = [&](auto dummy) {
        ...
        // Construct zstd dict
        // size_t localZstdDictSize;
        // ZSTD_CDict* const localZstdDict = ZSTD_createCDict(static_cast<const void*>(data_large.data()), localZstdDictSize, 3);
        // assert(localZstdDict != NULL);
    ...
    // codecs.push_back(std::make_unique<TurboPForCodec>(3));
    ...
    // codecs.push_back(std::make_unique<TwoIBenchInterpolativeBlockCodec>());
    // codecs.push_back(std::make_unique<TwoIBenchInterpolativeBlockParallelCodec>());

    // codecs.push_back(std::make_unique<TwoIBenchQmxBlockCodec>());
    // codecs.push_back(std::make_unique<TwoIBenchQmxBlockParallelCodec>());
