...
int main() {
  ...
  // We pick a CODEC
  std::cout << "BP32" << std::endl;
  IntegerCODEC &codec = *factory.getFromName("vsencoding");
  ...
  //
  int N = 65536;
  std::vector<int32_t> mydata(N);
  for (int32_t i = 0; i < N; i += 150)
    mydata[i] = i;
  //
  ...
  codec.encodeArray(reinterpret_cast<const uint32_t *>(mydata.data()), mydata.size(), compressed_output.data(),
                    compressedsize);
  //
  ...
  Delta::deltaSIMD(reinterpret_cast<uint32_t *>(mydata.data()), mydata.size());
  Delta::inverseDeltaSIMD(reinterpret_cast<uint32_t *>(mydata.data()), mydata.size());
