...
void decompressAndProcessWindow(CompressedDataList* compressed, FILE_DTYPE* decompressed) {
    CompressedNode* node = compressed->head;

    // sum = 0;
    // sampleSize = 0;

    while (node) {
        uint8_t *nodeData = node->compressedData;
        size_t nodeCompressedSize = node->compressedSize;
        size_t nodeDecompressedSize = node->uncompressedSize;
        // printf("Decompressing node with size %lu (decompressed: %lu). Next: %d \n", nodeCompressedSize, nodeDecompressedSize, node->next == NULL);
        // Decompress the node.
        // FILE_DTYPE* out_buf = (FILE_DTYPE *)malloc((nodeDecompressedSize + 32) * sizeof(FILE_DTYPE));
        TURBOP4_DEC(nodeData, nodeDecompressedSize, decompressed);
        // size_t nodeDataOffset = 0;
        // printf("%lu %lu \n", nodeDecompressedSize, nodeCompressedSize);
        // while (nodeDataOffset < nodeCompressedSize) {
        //     // printf("Decompress \n");
        //     // printf("o1 %lu \n", nodeDataOffset);
        //     size_t numDec = TURBOP4_DEC(&nodeData[nodeDataOffset], nodeCompressedSize - nodeDataOffset, out_buf); // Decompress window
        //     nodeDataOffset += numDec;
        // }
        // printf("f \n");
        // printf("Decompressed node \n");
        // The node has been decompressed in `decompressed`.
        // Perform write operation.
        linearSweepWrite(decompressed, nodeDecompressedSize);

        // free(node->compressedData);
        // node->compressedData = (uint8_t*)malloc(P4NENC_BOUND(nodeDecompressedSize));
        // Recompress the node.
        #if WRITE
            node->compressedSize = TURBOP4_ENC(decompressed, nodeDecompressedSize, node->compressedData); // Compress
        #endif
...
int main() {
    ...
    size_t blockSize = closest_multiple_less_than(48000); // Size of decompression window
    ...
    for (int rep = 0; rep < 1; rep++) {
        ...
        // sum = 0;
        // sampleSize = 0;
        decompressAndProcessWindow(&compressedList, decompressed);
        // printf("Mean: %f \n", sum / (double)sampleSize);
