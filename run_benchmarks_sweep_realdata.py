...
RSS_THRESHOLDS_MB = [1, 4, 8, 16, 64, 256, 512, 1024, 2048]
...
def run_one(tif, blocksize, numblocks, codec, atrans):
    ...
    # Split on benchmark block markers; each band emits one block.
    ...
    for block in blocks[1:]:  # skip text before first marker
        ...
        results.append({
            ...
            "codec": codec, "atrans": atrans, "band": band,
...
def main():
    ...
    for i, (tif, bs, nb, rss_mb, codec, atrans) in enumerate(runs):
        ...
        for r in run_one(tif, bs, nb, codec, atrans):
            r["target_rss_mb"] = rss_mb
            results.append(r)
            sum_str = f"{r['sum_ns']:.1f}" if r['sum_ns'] is not None else "N/A"
            print(f"  band={r['band']}  sum={sum_str} ns  RSS={r['rss_kb']} kB", flush=True)
