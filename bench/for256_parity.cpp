// CR-parity harness for the 256-bit fused FoR codecs.
//
// Compares each new vectorized codec against the equivalent non-vectorized
// composite (logical FoR → SimdCompFusedCodecU16 256-bit physical), tile-by-tile
// over the 3 local test TIFs, and reports the byte ratio vec/ref.
//
//   Regular FoR:    SimdCompFusedForCodecU16_256(w, sep)
//                   vs Composite(FORCodecU16(w, sep), SimdCompFusedCodecU16)
//   Hierarchical:   SimdCompFusedForHierarchicalCodecU16_256(gw, lw)
...
static std::unique_ptr<Codec> RefComposite(std::unique_ptr<Codec> logical) {
  return std::make_unique<CompositeStatefulIntegerCodec<uint16_t>>(
      std::move(logical), std::make_unique<SimdCompFusedCodecU16>());
}
...
static std::vector<Cfg> MakeConfigs() {
  ...
  for (size_t w : {4u, 8u, 16u, 32u, 64u, 128u, 256u}) {
    for (bool sep : {false, true}) {
      ...
      c.vec = std::make_unique<SimdCompFusedForCodecU16_256>(w, sep);
  ...
  for (size_t gw : {128u, 256u}) {
    for (size_t lw : {4u, 8u, 16u, 32u, 64u, 128u, 256u}) {
      ...
      c.vec = std::make_unique<SimdCompFusedForHierarchicalCodecU16_256>(gw, lw);
      c.ref = RefComposite(
